      ...
      >
        ...
        <header className="flex justify-between items-center mb-12">
          ...
          <button
            type="button"
            onClick={toggleFlash}
            className={`rounded-full p-2 transition-colors ${flashOn ? 'bg-amber-100 text-amber-600' : 'text-gray-500 hover:bg-gray-100'}`}
            aria-label={flashOn ? 'Flash on' : 'Flash off'}
            aria-pressed={flashOn}
          >
            <Zap size={28} strokeWidth={1.5} className={flashOn ? 'text-amber-600' : ''} />
          </button>
        </header>
    ...
    >
      ...
      <div className="px-6 pb-12 flex flex-col gap-10">
        <div className="w-full max-w-[240px] mx-auto aspect-[3/4] bg-gray-100 rounded-lg overflow-hidden shadow-xl rotate-[-2deg]">
          <img
            src={selectedFileUrl ?? 'https://picsum.photos/seed/receipt/400/600'}
            alt={selectedFile ? `Receipt preview: ${selectedFile.name}` : 'Receipt'}
