...
function isFocusableVisible(el: HTMLElement): boolean {
  ...
  if (pos === 'fixed' || pos === 'sticky') {
    ...
    return hasSize && inViewport && computed.visibility !== 'hidden' && Number(computed.opacity) > 0;
  }
  return false;
}
...
export function useOverlayAccessibility(
  ...
  useEffect(() => {
    ...
    const focusFirst = () => {
      ...
      if (!root) return;
      const nodes = [...root.querySelectorAll<HTMLElement>(FOCUSABLE)].filter((el) => isFocusableVisible(el));
      const first = nodes[0];
      first?.focus();
    };
