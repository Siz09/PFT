...
type OCRPorts = {
  ...
  scanWithTesseract: (input: OCRScanInput) => Promise<string>;
};
...
export const parseOcrResponse = (rawText: string): OCRExtractedFields => {
...
export class OCRService {
  ...
  async scan(input: OCRScanInput): Promise<OCRScanResult> {
    ...
    if (online) {
      ...
      const fields = parseOcrResponse(rawOpenAIResponse);
      return {
        provider: 'openai',
        fields,
    ...
    const rawTesseractResponse = await this.ports.scanWithTesseract(input);
    const fields = parseOcrResponse(rawTesseractResponse);
